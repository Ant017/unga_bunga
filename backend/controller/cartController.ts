...
class CartController {
  ...
  async paymentSuccess(req: Request, res: Response) {
    try {
      ...
      return res.redirect(
        `${appConfig.frontendUrl}/paymentSuccess/${order._id}`
      );
    } catch (error) {
      console.log(error);
      return res.status(500).send(failure("Internal server error", error));
  ...
  async getAllOrders(req: Request, res: Response): Promise<Response> {
    try {
      ...
      if (orders.length === 0) {
        return res.status(404).send(failure("No orders found"));
      }

      const filteredOrders = orders.filter(
        (order: IOrder) => order.userID !== null
      );

      return res
        .status(200)
        .send(success("Orders fetched successfully", filteredOrders));
    } catch (error) {
      console.log(error);
      return res.status(500).send(failure("Internal server error", error));
    }
  ...
  async getMyOrders(req: Request, res: Response): Promise<Response> {
    try {
      const customRequest = req as IAuthMiddleware;
      const orders = await orderModel
        .find({ userID: customRequest.user })
        .populate({
          path: "items.itemID",
          model: "Item",
        });
      ...
      return res
        .status(200)
        .send(success("Orders fetched successfully", orders));
    } catch (error) {
      console.log(error);
      return res.status(500).send(failure("Internal server error", error));
    }
  ...
  async getOrderById(req: Request, res: Response): Promise<Response> {
    try {
      ...
      return res.status(200).send(success("Order fetched successfully", order));
    } catch (error) {
      console.log(error);
      return res.status(500).send(failure("Internal server error", error));
    }
